    ...
    def is_allowed(self, client_id: str) -> tuple[bool, Optional[str]]:
        ...
        if client_id in self.cooldown_until:
            if current_time < self.cooldown_until[client_id]:
                ...
                # Cooldown expired, but keep violation count
        ...
        if len(timestamps) >= self.config.max_messages_per_minute:
            ...
            if violations >= 3:
                ...
                self.cooldown_until[client_id] = (
                    current_time + self.config.cooldown_period
                )
                return (
                    False,
                    f"Too many violations. Cooldown for {self.config.cooldown_period}s",
                )
            elif violations >= 2:
...
class ConnectionRateLimiter:
    ...
    def can_connect(
        self, ip_address: str, client_id: str
        ...
        if (
            len(attempts) >= self.config.max_connections_per_ip * 2
        ):  # 2x the limit for attempts
        ...
        if current_connections >= self.config.max_connections_per_ip:
            return (
                False,
                f"Maximum {self.config.max_connections_per_ip} connections per IP",
            )
