...
class TestLockCRUDOperations:
    ...
    def test_get_empty_project_locks(
        ...
        response = test_client.get(
            "/api/projects/empty_project/locks", 
    ...
    def test_create_component_lock(
        self, test_client: TestClient, auth_headers: Dict[str, str], sample_component_lock: Dict[str, Any]
    ) -> None:
        """
    ...
    def test_lock_operations_require_authentication(
        ...
        response = test_client.delete(f"/api/projects/{project_id}/locks/{component_id}")
        assert response.status_code == 403
...
class TestLockStateManagement:
    ...
    def test_lock_ownership_validation(
        ...
        if response.status_code == 403:
            assert "Cannot delete frozen lock owned by another user" in response.json()["detail"]
    ...
    def test_resolve_conflict(
        self, test_client: TestClient, auth_headers: Dict[str, str], sample_conflict: Dict[str, Any]
    ) -> None:
        """
...
class TestLockSystemEdgeCases:
    ...
    def test_lock_with_invalid_data_types(
        self, test_client: TestClient, auth_headers: Dict[str, str], invalid_data: Dict[str, Any]
    ) -> None:
        """
    ...
    def test_nonexistent_project_handling(
        ...
        response = test_client.get(
            f"/api/projects/{nonexistent_project}/locks",
            headers=auth_headers
        )
...
class TestWebSocketLockBroadcasts:
    ...
    def test_websocket_connection_with_valid_token(
        ...
        try:
            payload = jwt.decode(websocket_token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
    ...
    def test_websocket_lock_update_broadcast_format(self) -> None:
        ...
        lock = data["lock"]
        required_fields = ["id", "componentId", "level", "type", "reason", "lockedBy", "lockedAt"]
