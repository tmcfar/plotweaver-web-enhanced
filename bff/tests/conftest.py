...
try:
    ...
    async def create_or_update_lock(
        ...
        if existing_lock and existing_lock["level"] == "frozen" and existing_lock["lockedBy"] != payload.get("sub"):
    ...
    async def delete_lock(
        ...
        if project_id not in project_locks or component_id not in project_locks[project_id]:
            raise HTTPException(status_code=404, detail="Lock not found")
        
        existing_lock = project_locks[project_id][component_id]
        
        # Check ownership for frozen locks
        if existing_lock["level"] == "frozen" and existing_lock["lockedBy"] != payload.get("sub"):
            raise HTTPException(status_code=403, detail="Cannot delete frozen lock owned by another user")
            ...
            for component_id in operation.componentIds:
                try:
                    if operation.type == "lock":
                        ...
                        project_locks[project_id][component_id] = lock_data
                        results.append({"componentId": component_id, "status": "locked", "lock": lock_data})
                    
                    elif operation.type == "unlock":
                        if component_id in project_locks[project_id]:
                            del project_locks[project_id][component_id]
                        results.append({"componentId": component_id, "status": "unlocked"})
                    
                    elif operation.type == "change_level":
                        if component_id in project_locks[project_id]:
                            project_locks[project_id][component_id]["level"] = operation.lockLevel
                            results.append({"componentId": component_id, "status": "level_changed", "newLevel": operation.lockLevel})
                        else:
                            results.append({"componentId": component_id, "status": "not_found"})
                
                except Exception as e:
                    results.append({"componentId": component_id, "status": "error", "error": str(e)})
        
        return {"success": True, "results": results}
    
    @app.get("/api/projects/{project_id}/conflicts")
    async def get_project_conflicts(project_id: str, payload: dict = Depends(verify_token)):
    ...
    async def resolve_conflict(
        ...
        conflict_index = next((i for i, c in enumerate(conflicts) if c["id"] == conflict_id), None)
    ...
    async def check_lock_conflicts(
        ...
        for component_id in components:
            if component_id in project_locks_dict:
                ...
                can_override = existing_lock.get("canOverride", False) and existing_lock["level"] != "frozen"
...
def malformed_tokens() -> Dict[str, str]:
    ...
    return {
        "invalid_signature": jwt.encode(valid_payload, "wrong-secret", algorithm=JWT_ALGORITHM),
        "no_bearer_prefix": jwt.encode(valid_payload, JWT_SECRET, algorithm=JWT_ALGORITHM),
        "invalid_format": "this.is.not.a.jwt.token.at.all",
        "empty_token": "",
        "null_token": None,
        "corrupted_token": jwt.encode(valid_payload, JWT_SECRET, algorithm=JWT_ALGORITHM)[:-10] + "corrupted",
        "wrong_algorithm": jwt.encode(valid_payload, JWT_SECRET, algorithm="HS512"),
        "missing_claims": jwt.encode({"sub": "test"}, JWT_SECRET, algorithm=JWT_ALGORITHM),
    }
