...
class SecurityAudit:
    ...
    def add_passed(self, check_name: str, description: str):
        ...
        self.passed_checks.append({"check": check_name, "description": description})
                    ...
                    for pattern, description in patterns:
                        if re.search(pattern, line, re.IGNORECASE):
                            ...
                            if "change-in-production" in line or "your-secret" in line:
                                ...
                                self.add_issue(
                                    "HIGH", str(file_path), line_num, description
                                )
            except Exception:
                pass
    ...
    def check_sql_injection(self):
        ...
        if not found_issues:
            self.add_passed(
                "SQL_INJECTION", "No SQL injection vulnerabilities detected"
            )
    ...
    def check_authentication_implementation(self):
        ...
        if not auth_files:
            self.add_issue(
                "HIGH", "project", 0, "No authentication implementation found"
            )
            return
        ...
        for file_path in auth_files:
            try:
                ...
                if re.search(r"verify\s*=\s*False", content):
                    self.add_issue(
                        "HIGH", str(file_path), 0, "JWT verification disabled"
                    )
    ...
    def check_websocket_security(self):
        ...
        if auth_required:
            ...
            self.add_issue(
                "HIGH", "websocket", 0, "WebSocket authentication not implemented"
            )
        ...
        if rate_limiting:
            ...
            self.add_issue(
                "MEDIUM", "websocket", 0, "WebSocket rate limiting not implemented"
            )
        ...
        if message_validation:
            ...
            self.add_issue(
                "MEDIUM", "websocket", 0, "WebSocket message validation not implemented"
            )
                ...
                for line_num, line in enumerate(lines, 1):
                    for pattern, description in patterns:
                        if re.search(pattern, line):
                            ...
                            context = "\n".join(
                                lines[max(0, line_num - 3) : line_num + 3]
                            )
                            if not re.search(
                                r"valid|schema|BaseModel", context, re.IGNORECASE
                            ):
